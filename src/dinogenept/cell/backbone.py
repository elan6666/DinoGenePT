...
class KDAMixer(nn.Module):
    ...
    def forward(self, x, valid, writable):
        ...
        q = F.normalize(F.silu(self.q_proj(x)).reshape(shape).float(), dim=-1, eps=1e-6)
        k = F.normalize(F.silu(self.k_proj(x)).reshape(shape).float(), dim=-1, eps=1e-6)
        v = F.silu(self.v_proj(x)).reshape(shape)
...
class CellBackbone(nn.Module):
    ...
    def forward(self, gene_ids, expression, valid, hidden=None, prefix_tokens=None):
        ...
        result = self.norm(self.output_read(prefix, completed)) * all_valid.unsqueeze(-1)
    ...
    def lora_targets(self):
        ...
        for i, block in enumerate(self.blocks):
            leaves = (
                ...
                if isinstance(block.mixer, KDAMixer)
