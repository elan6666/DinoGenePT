...
Teacher/observed bags are independent draws from TRAIN only. Evaluation control
...
class PopulationIndex:
    def __init__(self, row_ids, conditions, contexts, splits):
    ...
    def training_bags(self, *, epoch, seed=42, bag_size=8):
        ...
        for (condition, context), rows in sorted(self.groups.items()):
            if condition == "ctrl" or condition not in self.splits["train"]:
                continue
            group_id = f"{condition}\x1f{context}"
            ...
            for j, primary in enumerate(np.array_split(shuffled, count)):
                bags.append(
                    TrainingBag(
                        ...
                        observed_post=self._draw(rng, rows, bag_size),
                        teacher_post=self._draw(rng, rows, bag_size),
                    )
                )
        if not bags:
