...
def test_chunk_matches_recurrence_values_and_gradients(size, implementation):
    ...
    actual, actual_state = implementation(*inputs, chunk_size=size)
...
def test_read_only_tokens_do_not_change_state_or_later_output(implementation):
    ...
    g[:, 2] = 0
    beta[:, 2] = 0
    full, state = implementation(q, k, v, g, beta, chunk_size=4)
    keep = torch.tensor([0, 1, 3, 4])
    short, short_state = implementation(q[:, keep], k[:, keep], v[:, keep], g[:, keep], beta[:, keep])
...
def test_strong_decay_does_not_create_future_overflow(implementation):
    ...
    output, state = implementation(q, k, v, g, beta, chunk_size=64)
